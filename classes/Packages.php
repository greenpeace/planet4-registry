 ...
 *
 * @copyright 2017 Greenpeace
 * @package Planet 4 Registry
 **/
...
class Packages {
	 ...
	 * @var string
	 **/
	const SatisFileExtraKey = 'satis-file';
	 ...
	 * @var string
	 **/
	const SatisExtendedFileExtraKey = 'satis-extended-file';
	 ...
	 * @var string
	 **/
	const PackageDirectoryExtraKey = 'packages-directory';
	...
	public static function extendSatisFile( Event $event ) {
		...
		$satisFile = isset( $extra[ self::SatisFileExtraKey ] )
			? $extra[ self::SatisFileExtraKey ]
			: 'satis.json';
		$satisExtendedFile = isset( $extra[ self::SatisExtendedFileExtraKey ] )
			? $extra[ self::SatisExtendedFileExtraKey ]
			: 'satis.extended.json';
		$packagesDirectory = isset( $extra[ self::PackageDirectoryExtraKey ] )
			? $extra[ self::PackageDirectoryExtraKey ]
			: 'packages';
	...
	public function combine() {
		...
		// todo: add some checkes that will prevent duplicate repositories
		$satis['repositories'] = array_merge( $satis['repositories'], $this->packagesFromDirectory( $this->packagesDirectory ) );
	 ...
	 * sub-directories.
	 *
	 * @return string $directory The directory to inspect
	 * @return array The version files found inside the given directory.
	 **/
	public function versionFilesFromDirectory( $directory ) {
	 ...
	 **/
	public function packagesFromFile( $file ) {
	 ...
	 *
	 * This method will walk recursivly through a directory of package files
	 ...
	 **/
	public function packagesFromDirectory( $directory ) {
		$result = array();
		$versionFiles = $this->versionFilesFromDirectory( $this->packagesDirectory );
